...
- `src/store/store.js` — Zustand store with `persist`; only manages `mode` (dark/light), saved to localStorage under key `'cv'`.
...
- `src/locales/es.js` — **Single source of truth for all UI text** (name, nav items, job history, education, contact info, footer). Import via `import { t } from '../locales'`.
...
All display content lives in `src/locales/es.js`. To add a language: create `locales/en.js` with the same structure, import it in `locales/index.js`, add it to the `locales` object, and update `defaultLocale`.

### Routing

Hash router — no server configuration needed. Routes: `/`, `/experiencia`, `/estudios`, `/herramientas`, `/proyectos`, and a wildcard `*` for 404.
